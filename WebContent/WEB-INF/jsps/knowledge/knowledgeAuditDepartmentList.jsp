...
<html>
	...
	<body>
		...
		<div class="container">
			<h5 class="dangqwz">当前位置：系统管理 > 知识管理 > <span class="redspan">部门知识审核</span> </h5>
			<div class="syswhite yigebiao">
				<h4 class="biaoti">知识审核</h4>
				<hr class="clearFloat" />
				<form class="form-horizontal" role="form" action="/admin/knowledge/auditDepartmentList" method="get">
					...
					<tbody align="center">
						<c:forEach var="knowledge" items="${knowledgeDtoList}" varStatus="s">
							<tr>
								...
								<td><a href="/admin/knowledge/audit/details/department?id=${knowledge.id}" class="btn btn-primary btn-xs btn-color">查看详情</a></td>  
							</tr>
						</c:forEach>
					</tbody>
				</table>
				</form>
				<ghs:pager url="/admin/knowledge/auditDepartmentList?title=${title }&createUser=${createUser }&status=${status }" current="${pageIndex }" total="${totalCount }" pageSize="<%=com.ghs.kbvs.constants.KbvsConstant.ADMIN_PAGE_SIZE_10 %>" />
			</div>
		</div>
