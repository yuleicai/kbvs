...
<html>
	...
	<body>
		...
		<div class="container">
			<h5 class="dangqwz">当前位置：系统管理 > <a href="/admin/documents/list">文档中心管理 </a>> <span class="redspan">文档列表</span> </h5>
			<div class="syswhite yigebiao">
				<h4 class="biaoti">文档列表</h4>
				<hr class="clearFloat" />
				<form id="listForm" action="${ctx}/admin/documents/list" method="get" class="form-horizontal" role="form">
...
<%-- 								<c:if test="${documents.subProcessCid !=-1 }"> --%>
...
<%-- 								</c:if> --%>
						...
						<div class="col-sm-6 ">
						<!-- 							onclick="jQuery.location('/admin/documents/publish')" -->
							<button type="button" class="btn btn-default btn-width float-rightt" 
								onclick="javascript:window.location.href='/admin/documents/publish'"
								>发布文档</button>&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<table data-toggle="table">
			         	...
			         	<tbody class="text-center">
				            <c:forEach var="entity" items="${list}" varStatus="s">
							     <tr>                         
									<td   title="${s.count}" class="col-sm-1 rowinline text-center" >${s.count }</td>
									<td   title="${entity.name}">${entity.name}</td>
									<td   title="${entity.docType.name}" class="col-sm-1 rowinline text-center" >${entity.docType.name}</td>
									<td   title="${entity.viewCount}" class="col-sm-1 rowinline text-center" >${entity.viewCount}</td>
									<td   title="${entity.createUser.name}(${entity.createUser.email})" class="col-sm-2 rowinline text-center" >${entity.createUser.name}(${entity.createUser.email})</td>
								    <td   title="<fmt:formatDate type="time" value="${entity.createTime}" pattern="yyyy-MM-dd HH:mm:ss" />" class="col-sm-2 rowinline text-center" ><fmt:formatDate type="time" value="${entity.createTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
								    <td class="caozuo col-sm-1 rowinline text-center">
								    	<a class="btn btn-primary btn-xs btn-color" href="${ctx}/admin/documents/details?id=${entity.id}" target="_blank">查看详情</a>
								    </td> 
							    </tr>                        
				           	</c:forEach>
			            </tbody>
		         	</table>
		         	<ghs:pager url="/admin/documents/list?name=${documents.name}&code=${documents.code}&docTypeId=${documents.docTypeId}&companyId=${documents.companyId}&deptId=${documents.deptId}&docLevelId=${documents.docLevelId}&docScopeId=${documents.docScopeId}&subProcessPid=${documents.subProcessPid}&subProcessCid=${documents.subProcessCid}"  
