...
<html>
	...
	<body>
		...
		<div class="container">
			<h5 class="dangqwz">当前位置：系统管理  ><a href="/admin/notice/module/list">公告管理 </a> ><span class="redspan">部门公告列表</span> </h5>
			<div class="syswhite yigebiao">
				<h4 class="biaoti">部门公告列表</h4>
				<hr class="clearFloat" />
				<form id="listForm" role="form" action="/admin/notice/departmentList" method="get" class="form-horizontal" role="form">
					<div class="form-group">
						...
						<div class="col-sm-2">
							<select class="form-control" id="sonModuleId" name="sonModuleId">
								<option value="0" <c:if test="${noticeDto.sonModuleId==0 }">selected="selected"</c:if>>全部</option>
								<c:forEach items="${noticeSonModuleList}" var="entity">
									<option value="${ entity.id}"     
										<c:if test="${noticeDto.sonModuleId==entity.id }">selected="selected"</c:if>>${ entity.name}
									</option>
								</c:forEach>
							</select>
						</div>
					...
					<table data-toggle="table">
			         	...
			         	<tbody>
				            <c:forEach var="list" items="${noticeList}" varStatus="s">
							     <tr>                         
									...
									<td title="${list.sonModuleName}" class="col-sm-1 rowinline text-center">${list.sonModuleName}</td>
		         	...
		         	<ghs:pager url="/admin/notice/departmentList?title=${noticeDto.title}&moduleId=${noticeDto.sonModuleId}&isEnable=${noticeDto.isEnable}&isTop=${noticeDto.isTop}&startTime=${noticeDto.param}"	
