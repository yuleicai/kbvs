...
<html>
	...
	<body>
		...
		<div class="container">
			<h5 class="dangqwz">当前位置：我的知识库 > <span class="redspan">申请发布</span> </h5>
			<div class="syswhite yigebiao">
				<h4 class="biaoti">申请发布</h4>
				<hr class="clearFloat" />
							...
							<div class="form-group" id="div0_span_1">
								<span id="span_1">
									<div class="col-sm-2" id = "div_span_1_1">
										...
										<select class="form-control" id="span_1_1" name="categoryName" onchange="jQuery.knowledge.addMyChangeCategory(this)">   
											<c:forEach items="${categoryList}" var="categoryBean">
												<option 
													<c:choose>
														<c:when test="${categoryBean['tempId']==0 }"> value=-1</c:when>
														<c:otherwise>  value="${categoryBean.tempId}"</c:otherwise>
													</c:choose> >${categoryBean.name}</option>
											</c:forEach>
										</select>&nbsp;&nbsp;
									</div>
								</span>
				...
				<div class="form-group">
					<div class="col-sm-offset-4 col-sm-6">
						<button type="button" class="btn btn-default btn-width" onclick="jQuery.knowledge.getCheckLabel();jQuery.knowledge.addMyKnowledge('saveForm')">发 布</button>
					</div>
				</div>
