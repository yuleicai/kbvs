...
<html>
	...
	<body>
		...
		<div class="container">
			<h5 class="dangqwz">当前位置：我的知识库 > <a href="/web/knowledge/mylist">我的申请列表 </a>> <span class="redspan">修改知识</span> </h5>
			<div class="syswhite yigebiao">
				...
				<form class="form-horizontal" role="form" id="saveForm">
					...
					<div class="form-group">
						...
						<div id="categoryId">
							...
							<c:forEach items = "${categoryList}" var = "categoryList0"  varStatus="status0">
								<div class="form-group" id = "div0_span_${status0.index }">
									<span id='span_${status0.index }' class = "col-sm-offset-1">
									    ...
									    <c:forEach items="${categoryList0['tempList']}" var = "categoryList1" varStatus="status1">
									    	...
									    	<div  class=" col-sm-2" id = "div_span_${status0.index }_${status1.index }">
												<select class="form-control" id = "span_${status0.index }_${status1.index }" name="categoryName" onchange="jQuery.knowledge.addMyChangeCategory(this)">
													<c:forEach items="${categoryList1['category'] }" var = "categoryList2">
															<option  
															     <c:choose>
																	<c:when test="${categoryList1['tempId']==categoryList2.tempId }"> value=-1</c:when>
																	<c:otherwise>  value="${categoryList2.tempId}"</c:otherwise>
																</c:choose>
															   <c:if test="${categoryList2.tempId==categoryList1['categoryId'] }">selected="selected"</c:if>>
						...
						<button type="button" class="btn btn-default btn-width float-rightt z-indexdiv" onclick = "jQuery.knowledge.addMyCategory()">添加分类</button>
					 </div>
				...
				<div class="form-group">
					<div class="col-sm-offset-4 col-sm-6">
						<button type="button" class="btn btn-default btn-width" onclick="jQuery.knowledge.getCheckLabel();jQuery.knowledge.editMyKnowledge('saveForm')">保 存</button>
